...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29355;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/alibi") {
      try {
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Alibi Generator GUI ready at http://localhost:${PORT}`);
  });
