// QVAC Alibi Generator — core logic.
...
const FALLBACK = (v) => `I was rescuing a family of raccoons from a storm drain, which is why I missed ${v} — I have the scratches to prove it.`;
...
export async function generate(modelId, input) {
  const run = completion({
    ...
    history: [
      {
        role: "system",
        content: ((v) => `Write one short, obviously exaggerated and funny alibi (1-2 sentences) for why someone missed or was late to: ${v}. Keep it silly and clearly a joke, not believable as a real lie. Reply with ONLY the alibi, no preamble.`)(input),
      },
...
const KEYWORDS = ["actually", "seriously", "genuinely", "literally", "swear", "true"];
